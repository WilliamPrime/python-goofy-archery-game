...
def runRound():
    ...
    if checker(targetX,targetY,targetHeight,target)==True:
        ...
        for i in range(targetX+1,boardColumns):
            for j in range(0,boardRows+1):
                oRenderer.editSquare((i),(j),emptyCell)
...
while playing==True:
    if playing==True:
        ...
        score=score+runRound()
        if roundNumber==1:
            print("in ",roundNumber," round you scored, ", score)
        else:
            print("in ",roundNumber," rounds you scored a total of ",score)
        
        keepPlaying=""
        keepPlaying=str(input("do you want to keep playing?? yes or no "))
        if keepPlaying.lower()=="yes":
            ...
            playing==False
            print("im sorry to see you leave i hope you had fun")
            break
